            ...
            $.post(this.options.url, param, function(data) {
                ...
                $.each(self.dataList, function(i) {
                    if (typeof self.options.template === "function") {
                        if (self.options.paging) self.$tbody.append(self.options.template(i, this, (self.page_options.currentPage - 1) * self.options.pageSize + i + 1)); else self.$tbody.append(self.options.template(i, this));
                    }
                });
                $(".alt").tooltip({
                    placement:"bottom"
